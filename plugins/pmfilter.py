...
async def next_page(bot, query):
    ...
    if settings.get('button'):
        ...
        btn.insert(0,
                   [
                       InlineKeyboardButton(
                           "#𝐌𝐨𝐯𝐢𝐞𝐬𝐑𝐞𝐪𝐮𝐞𝐬𝐭🌈", url=f"https://t.me/Deendayal_dhakad_Contact"),
                       InlineKeyboardButton(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('⫸ Sᴇᴀʀᴄʜ Mᴏᴠɪᴇs / Sᴇʀɪᴇs Hᴇʀᴇ ⫷', url=f"https://t.me/Noobflix_Hindi_Movies")
                ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
            InlineKeyboardButton('Main channel ⚡', url='https://t.me/Noobflixmovies'),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if not settings.get('button'):
            cap += "\n<b>♻️ ʀᴇꜱᴜʟᴛꜱ ꜰᴏʀ ʏᴏᴜʀ sᴇᴀʀᴄʜ 👇</b>"
            for idx, file in enumerate(files, start=1):
                cap += f"<b>\n{idx}. <a href='https://telegram.me/{temp.U_NAME}?start=file_{message.chat.id}_{file.file_id}'>📕 {get_size(file.file_size)} | {clean_filename(file.file_name)}\n</a></b>"
    else:
        if settings.get('button'):
            cap = f"<b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n📚 ᴀʟʟ ꜰɪʟᴇꜱ : <code>{total_results}</code>\n⏰ ᴠɪᴇᴡ ʀᴇsᴜʟᴛ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n🙋‍♂ Hᴇʏ : {message.from_user.mention}\n⚜️ ᴘᴏᴡᴇʀᴇᴅ ʙʏ : ⚡ {message.chat.title or temp.B_LINK or '@Deendayal_dhakadd'} \n♻️ ʀᴇꜱᴜʟᴛꜱ ꜰᴏʀ ʏᴏᴜʀ sᴇᴀʀᴄʜ 👇\n\n</b>"
        else:
            cap = f"<b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n📚 ᴀʟʟ ꜰɪʟᴇꜱ : <code>{total_results}</code>\n⏰ ᴠɪᴇᴡ ʀᴇsᴜʟᴛ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n🙋‍♂ Hᴇʏ : {message.from_user.mention}\n⚜️ ᴘᴏᴡᴇʀᴇᴅ ʙʏ : ⚡ {message.chat.title or temp.B_LINK or '@Deendayal_dhakadd'} \n♻️ ʀᴇꜱᴜʟᴛꜱ ꜰᴏʀ ʏᴏᴜʀ sᴇᴀʀᴄʜ 👇\n\n</b>"
